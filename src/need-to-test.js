...
async function getJwksKey(kid) {
  ...
  // Okta usually provides x5c; we can turn that into a PEM cert.
  if (!key.x5c || !key.x5c.length) {
    throw new Error('JWKS key missing x5c certificate chain');
  }
...
async function verifyIdToken(idToken, expectedNonce) {
  ...
  if (!kid) throw new Error('id_token missing kid header');
